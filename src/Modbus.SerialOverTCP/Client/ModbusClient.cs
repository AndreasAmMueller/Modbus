...
using AMWD.Modbus.SerialOverTCP.Util;
using Microsoft.Extensions.Logging;

namespace AMWD.Modbus.SerialOverTCP.Client
{
	/// <summary>
	...
	public class ModbusClient : IModbusClient
	{
						...
						while (!receiveCts.IsCancellationRequested)
						{
							...
							using (var cts = CancellationTokenSource.CreateLinkedTokenSource(timeCts.Token, receiveCts.Token))
							{
								try
								{
									...
									// Device/Slave ID
									List<byte> responseBytes =new List<byte> ( await stream.ReadExpectedBytes(2, cts.Token));
									// Function number
									byte fn = responseBytes[1];
									...
									switch (function)
									{
										...
										case FunctionCode.ReadInputRegisters:
											expectedBytes = await stream.ReadExpectedByte(cts.Token);
										...
										case FunctionCode.EncapsulatedInterface:
											responseBytes.AddRange(await stream.ReadExpectedBytes(6, cts.Token));
											byte count = responseBytes.Last();
											for (int i = 0; i < count; i++)
											{
												// id
												responseBytes.Add(await stream.ReadExpectedByte(cts.Token));
												// length
												expectedBytes = await stream.ReadExpectedByte(cts.Token);
												responseBytes.Add(expectedBytes);
												// value
												responseBytes.AddRange(await stream.ReadExpectedBytes(expectedBytes, cts.Token));
											}
											expectedBytes = 0;
											break;
										default:
		...
		{
			CheckDisposed();
			return $"Modbus SerialOverTCP {Host}:{Port} - Connected: {IsConnected}";
		}
